...
namespace conectorPDV001
{
    public class geraNFe
    {
        ...
        public void cancelaNFe(dadosCancelaNfe cancelaNfe)
        {
            ...
            string caminho = Path.GetFullPath("cancelaNF");
            DirectoryInfo dirInfo = new DirectoryInfo(caminho);
            if (!dirInfo.Exists)
            {
                Directory.CreateDirectory(System.AppDomain.CurrentDomain.BaseDirectory + @"/cancelaNF");
                caminho = Path.GetFullPath("cancelaNF");
            }

            xmlNfe.Save(caminho + "\\" + cancelaNfe.chNFe + "-cancelada.xml");
            assinaturaNfe assinarNotaFiscal = new assinaturaNfe();
            assinarNotaFiscal.assinarNfe(caminho + "\\" + cancelaNfe.chNFe + "-cancelada.xml", "Signature", "NFe", cancelaNfe.chNFe);
            xmlNfe.Load(caminho + "\\" + cancelaNfe.chNFe + "-cancelada.xml");

            regrasNfe.enviaNotaFiscalServidor(xmlNfe, cancelaNfe.chNFe);
        }
        ...
        public dadosConsultaProcessamentoLote geraNotaFiscalEletronica(Int16 tipoMetodo,
            ...
            List<dadosEnvioNfeLote> listaEnviaLote)
        {
            ...
            try
            {
                ...
                if (tipoRequisicao == 1) //Recepção NFe
                {
                    ...
                    // ---------------------------------------------------------------- início nfeProc -- n1                
                    xmlNfeElement = xmlNfe.CreateElement("idLote");
                    //xmlNfeElement.InnerText = regrasNfe.validacaoPadraoCampos(EnvioNfeLote.idlote, 2, 1, 15, false, null, "idLote");//Flavio
                    xmlNfeElement.InnerText = regrasNfe.validacaoPadraoCampos(listaEnviaLote[0].idlote, 2, 1, 15, false, null, "idLote");
                    ...
                    xmlNfeElement.InnerText = regrasNfe.validacaoPadraoCampos(listaEnviaLote[0].indSinc, 2, 1, 1, false, null, "indSinc");
                ...
                for (int i = 0; i < contadorGlobal; i++)
                {
                                ...
                                if (regrasNfe.verificaTipoModelo55NFe(identificacaoNfe.ide_mod))
                                {
                                    if (listaImpostoProdutosIndustrializados.Count > 0)
                                    {
                                        ...
                                        if (regrasNfe.verificaIpiInformado(ImpostoProdutosIndustrializados))
                                        {
                                            ...
                                            xmlNfe.AppendChild(xmlNfeNode);
                                            //----------------------------------------------------------------- fim
                                            if (regrasNfe.verificaCampoVazio(ImpostoProdutosIndustrializados.IPI_IPITrib_CST))//Flavio
                                            {
                                            ...
                                            if (regrasNfe.verificaCampoVazio(ImpostoProdutosIndustrializados.IPI_IPINT_CST))//Flavio
                                            {
                    ...
                    if (!regrasNfe.verificaTipoModelo55NFe(identificacaoNfe.ide_mod))
                    {
                        for (int r = 0; r < listaFormasPagamento.Count; r++)
                        {
                            ...
                            if (r == 100)
                                break;
                            //if (listaFormasPagamento[r].numeroNotaFiscal.Equals(identificacaoNfe.ide_cNF))//Flavio
                            if (listaFormasPagamento[r].numeroNotaFiscal.Equals(identificacaoNfe.ide_nNF))
                            {
                                // ---------------------------------------------------------------- inicio pag -- YA01 - A01
                                ...
                                if (regrasNfe.verificaTipoFormaPagamento(FormasPagamento.pag_tPag.PadLeft(2, '0')))//Flavio
                                {
                                    xmlNfeElement = xmlNfe.CreateElement("tPag");
                                    switch (FormasPagamento.pag_tPag.PadLeft(2, '0'))
                                    {
                                        case "01":
                                            xmlNfeElement.InnerText = "01";
                                            break;
                                        case "02":
                                            xmlNfeElement.InnerText = "02";
                                            break;
                                        case "03":
                                            xmlNfeElement.InnerText = "99";
                                            break;
                                        case "04":
                                            xmlNfeElement.InnerText = "99";
                                            break;
                                    ...
                                    //xmlNfeElement.InnerText = FormasPagamento.pag_tPag.PadLeft(2, '0');
                    ...
                    if (listaInformacoesAdicionaisNFe.Count > 0)
                    {
                        InformacoesAdicionaisNFe = listaInformacoesAdicionaisNFe[0];
                        ...
                        {
                            InformacoesAdicionaisNFe = listaInformacoesAdicionaisNFe[i];
    ...
    public class regrasNegocioNfe
    {
        ...
        public bool verificaNomeDestinatario(string nome, string tipoNota)
        {
            ...
            {
                if (tipoNota.Equals("55") || tipoNota.Equals("65"))
                {
        ...
        public bool verificaMensagemRetornoRecepcao(XmlNode retornoXml)
        {
            ...
            XmlNodeList findxn = xml.GetElementsByTagName("protNFe");

            try
            {
                if (alwaysVariables.Entidade == "13")
                {
                    if (xml.GetElementsByTagName("cStat")[1].InnerText != null)
                        codigoMensagem = xml.GetElementsByTagName("cStat")[1].InnerText;

                    if (xml.GetElementsByTagName("xMotivo")[1].InnerText != null)
                        mensagemServidor = xml.GetElementsByTagName("xMotivo")[1].InnerText;

                    if (xml.GetElementsByTagName("nRec")[0].InnerText != null)
                        numeroRecibo = xml.GetElementsByTagName("nRec")[0].InnerText;

                    if (xml.GetElementsByTagName("digVal")[0].InnerText != null)
                        alwaysVariables.DigestValue = xml.GetElementsByTagName("digVal")[0].InnerText;

                    if (numeroRecibo.Trim() != "")
                    {
                        alwaysVariables.ProtocoloNfce = xml.GetElementsByTagName("nProt")[0].InnerText;
                        alwaysVariables.chaveNfce = xml.GetElementsByTagName("chNFe")[0].InnerText;
                        alwaysVariables.versaoNfce = xml.GetElementsByTagName("verAplic")[0].InnerText;
                        alwaysVariables.dataAutorizaNfce = xml.GetElementsByTagName("dhRecbto")[0].InnerText;
                        alwaysVariables.MotivoNfce = xml.GetElementsByTagName("xMotivo")[1].InnerText;
                        alwaysVariables.CodigoMsg = xml.GetElementsByTagName("cStat")[0].InnerText;
                    }
                    else
                    {
                        alwaysVariables.MotivoNfce = xml.GetElementsByTagName("xMotivo")[0].InnerText;
                        alwaysVariables.CodigoMsg = xml.GetElementsByTagName("cStat")[0].InnerText;
                    }
                }
                else if (alwaysVariables.Entidade == "31")
                {
                    foreach (XmlNode xn in xml)
                    {
                        ...
                        if (xn["nProt"] != null)
                            numeroRecibo = xn["nProt"].InnerText;

                        alwaysVariables.ProtocoloNfce = xn["nProt"].InnerText;//Flavio
                        alwaysVariables.chaveNfce = xn["chNFe"].InnerText;
                        alwaysVariables.versaoNfce = xn["verAplic"].InnerText;
                        alwaysVariables.dataAutorizaNfce = xn["dhRecbto"].InnerText;
                        alwaysVariables.MotivoNfce = xn["xMotivo"].InnerText;
                        alwaysVariables.CodigoMsg =xn["cStat"].InnerText;
                        alwaysVariables.DigestValue = xn["digVal"].InnerText;
                    }
                }
            }
            catch (Exception e)
            {
                alwaysVariables.MotivoNfce = mensagemServidor;
                alwaysVariables.CodigoMsg = codigoMensagem;
            }
        ...
        public static string GetXMLFromObject(object o)
        {
            ...
            XmlSerializer seri = new XmlSerializer(o.GetType());
            MemoryStream stream = new MemoryStream();
            TextWriter writer = new StreamWriter(stream);
            seri.Serialize(writer, o);

            stream.Position = 0;
            XmlTextReader reader = new XmlTextReader(stream);
            XElement xml = XElement.Load(reader, LoadOptions.None);
        ...
        public string NFeAutorizaNF(string XmlEnvio, string VersaoSchema, short Ambiente, short CodIbgeUfEmi, string SiglaWS, ref string MsgErr)
        {//Flavio Rogerio
            try
            {
                            ...
                            case 2: // HOMOLOGAÇÃO
                                ...
                                switch (CodIbgeUfEmi)
                                {
                                    case 31: // MG
                                        ...
                                        webserviceAutorizacaoLoteHomo.NfeAutorizacao wsMG = new webserviceAutorizacaoLoteHomo.NfeAutorizacao();
                                        webserviceAutorizacaoLoteHomo.nfeCabecMsg nfeCabecMsgMG = new webserviceAutorizacaoLoteHomo.nfeCabecMsg();
                                        webserviceAutorizacaoLoteHomo.nfeDadosMsg nfeDadosMsgMG = new webserviceAutorizacaoLoteHomo.nfeDadosMsg();
                                        ...
                                        if (colecaoCertificado.Count > 0)
                                        {
                                            ...
                                            wsMG.ClientCertificates.Add((assina.carregaCertificados())[0]);
                                            wsMG.Timeout = 120000;

                                            // Comunica com o WebService
                                            string objMG = nfeDadosMsgMG.Any[0].OuterXml;

                                            object[] results = wsMG.NfeAutorizacaoLote(nfeDadosMsgMG);

                                            return (((System.Xml.XmlNode[])(results[0]))[8]).InnerXml;
                                        }
                                        else
                                        {
                                    ...
                                    case 13: //AM
                                        br.gov.am.sefaz.homnfce.NfeAutorizacao wsAM = new br.gov.am.sefaz.homnfce.NfeAutorizacao();
                                        br.gov.am.sefaz.homnfce.nfeCabecMsg nfeCabecMsgAM = new br.gov.am.sefaz.homnfce.nfeCabecMsg();
                                        br.gov.am.sefaz.homnfce.nfeDadosMsg nfeDadosMsgAM = new br.gov.am.sefaz.homnfce.nfeDadosMsg();

                                        nfeCabecMsgAM.cUF = CodIbgeUfEmi.ToString().Trim();
                                        nfeCabecMsgAM.versaoDados = VersaoSchema;
                                        wsAM.nfeCabecMsgValue = nfeCabecMsgAM;
                                        nfeDadosMsgAM.Any = new XmlNode[] { oXmlDoc };
                                        nfeDadosMsgAM.Any[0] = oNode;

                                        // Carrega o certificado
                                        X509Certificate2Collection colecaoCertificadoAM = assina.carregaCertificados();
                                        // Se conseguiu localizar o certificado digital
                                        if (colecaoCertificadoAM.Count > 0)
                                        {

                                            // Coloca o certificado
                                            wsAM.ClientCertificates.Add((assina.carregaCertificados())[0]);
                                            wsAM.Timeout = 120000;

                                            // Comunica com o WebService
                                            string objAM = nfeDadosMsgAM.Any[0].OuterXml;

                                            XmlDocument Doc = new XmlDocument();
                                            Doc.LoadXml(objAM);

                                            return wsAM.nfeAutorizacaoLote(Doc.DocumentElement).OuterXml;
        ...
        public dadosConsultaProcessamentoLote enviaNotaFiscalServidor(XmlDocument documento, string chaveAcessoGeradaNfe)
        {
            ...
            if (alwaysVariables.Entidade == "31")
            {
            ...
            else if (alwaysVariables.Entidade == "13")
            {
            ...
            if (alwaysVariables.Entidade == "31")
            {
                ...
                xmlCompleto.Append("<soap:Envelope xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">");
                xmlCompleto.Append("<soap:Header>");
                xmlCompleto.Append("<nfeCabecMsg xmlns=\"http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao\">");
                xmlCompleto.Append("<cUF>13</cUF>");
                xmlCompleto.Append("<versaoDados>3.10</versaoDados>");
                xmlCompleto.Append("</nfeCabecMsg>");
                xmlCompleto.Append("</soap:Header>");
                xmlCompleto.Append("<soap:Body>");
                xmlCompleto.Append("<nfeDadosMsg xmlns=\"http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao\">");
                ...
                xmlCompleto.Append("</soap:Body>");
                xmlCompleto.Append("</soap:Envelope>");
                ...
                string teste = NFeAutorizaNF(documento.OuterXml, "3.10", 2, Convert.ToInt16(alwaysVariables.Entidade), "SEFAZEST", ref mens);
                //string teste = NFeAutorizacaoMG(xmlCompleto.ToString(), "3.10", 2, 31, "SEFAZEST", ref mens);


                XmlDocument myXmlDocument = new XmlDocument();
                myXmlDocument.LoadXml(teste);
                ...
                dadosConsulta.statusEnvio = verificaMensagemRetornoRecepcao(myXmlDocument.DocumentElement);
            ...
            {
                br.gov.am.sefaz.homnfce.NfeAutorizacao wsEnviaLoteNfe = new br.gov.am.sefaz.homnfce.NfeAutorizacao();

                string mens = "";

                string teste = NFeAutorizaNF(documento.OuterXml, "3.10", 2, 13, "SEFAZEST", ref mens);
            ...
            else if (alwaysVariables.Entidade == "43")
            {
